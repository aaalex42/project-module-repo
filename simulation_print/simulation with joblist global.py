...
class ProdPlan():
    ...
    def plan(self, now):
        ...
        #print(self.joblist)
        joblist_final.append(self.joblist)
...
if __name__ == '__main__':
    ...
    d1 = Demand(master_product_data[0], RBASE, DAYS_FC, env)      
    d2 = Demand(master_product_data[1], RBASE, DAYS_FC, env)
    avg_OEE = (d1.avg_daily_machine_time() + d2.avg_daily_machine_time()) / ONE_DAY
    print(f'Expected OEE = {avg_OEE}')
    inv1 = Inventory(d1, env)
    inv2 = Inventory(d2, env)
    demand_and_inv_pipeline = [{'d': d1, 'inv': inv1}, {'d': d2, 'inv': inv2}]
    ...
    env.process(fulfill(env, d1, inv1, ALLOW_PART_SHIPMENTS, ONE_DAY))
    env.process(fulfill(env, d2, inv2, ALLOW_PART_SHIPMENTS, ONE_DAY))
    ...
    printfunc_prodplan(pp)


    print("JOBLIST FINAL")
    for joblist in joblist_final:
        print(joblist, "\n")
    
    if True:
        import matplotlib.pyplot as plt
        plt.figure()
        plt.plot(inv1.t, inv1.levels, label='inventory 1')
        plt.plot(inv2.t, inv2.levels, label='inventory 2')
        plt.legend(loc = 'upper left')
        plt.title('Inventory')
        plt.show()

        print(f'Actual avg inventory 1 level = {round(inv1.avg_level())}')
        print(f'Actual avg inventory 2 level = {round(inv2.avg_level())}')
        print(f'Actual service level demand 1 = {d1.service_level()}')
        print(f'Actual service level demand 2 = {d2.service_level()}')
        OEE = pp.prod_time / (SIM_END - SIM_START)
        print(f'Actual OEE = {OEE}')    

        plt.figure() 
        plt.plot(d1.t, d1.d, label='demand')
        plt.plot(d1.t, d1.ff, label='fulfilled')
        plt.plot(d1.t, d1.delta_bl, label='delta backlog')
        plt.legend(loc = 'upper left')
        plt.title('Demand 1')
        plt.show()

        plt.figure() 
        plt.plot(d2.t, d2.d, label='demand')
        plt.plot(d2.t, d2.ff, label='fulfilled')
        plt.plot(d2.t, d2.delta_bl, label='delta backlog')
        plt.legend(loc = 'upper left')
        plt.title('Demand 2')
        plt.show()
