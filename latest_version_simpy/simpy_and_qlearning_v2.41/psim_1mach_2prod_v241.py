...
Changelog:
    ...
    V2.41 12.12.23 minor fixes like div by zero fix in Demand.service_level
          13.12.23 added success return val to fulfill class function
"""
...
"""
SIM_CYCLES = 100_000 #200 previously
SIM_START = 0.0
...
class Demand():
    ...
    def fulfill(self, mat, allow_part_shipments) -> int:
        ...
        13.12.23: return success = True if current demand could be fulfilled, False otherwise
        """
        mat_used = 0
        bl0 = self.backlog
        ...
        if self.demands[0] > 0:
            ...
            # if no shipment this run is considered good
            success = True
    ...
    def service_level(self):
        ...
        12.12.23: prevent div by zero
        """
        total = self.n_good + self.n_bad
        return self.n_good / total if total != 0 else 1
...
class Inventory():
    ...
    - this class also holds the information for replenishment handling: at the
      ...
      fulfillment routine counts down replen_open. This is how it works and implements
      a TOC rope:
          - replen_open > 0 indicates if a reorder cycle is started
          - replen_due <= 0 indicates if a reorder cycle is finished
          - at the beginning, both replen_due and replen_open are = 0 in Inventory:init
          - when the reorder point is passed and no replen cycle is finished,
            the ProdPlan::plan routine sets replen_open and replen_due to ROP - inv.level > 0,
            so that both variables describe the current replen amount necessary
          - if the EOQ is lower than this amount necessary, several productions need to
            be added by ProdPlan::plan, always reducing replen_open -= eoq, until
            replen_open < 0, then the replen cycle is finished with regard to new 
            production jobs
          - when material becomes available from production and is stored in inventory,
            replen_due is decremented by -= amount in the produce process wrapper (this
            is a bit inconsistent but there is no good safe place to do it otherwise).
            For as long as replen_due > 0 the replenishment cycle is considered under
            way and no other cycle can be started.
...
class ProdPlan():
    ...
    def plan(self, now):
        ...
        see a comment in the inventory class for explanation of the replen_due and
        _open parameters
        """
...
def fulfill(env, d, inv, allow_part_shipments, bucket):
    ...
    13.12.23: fix for new return value in d.fulfill
    """
    while True:
        # provide all material to demand, deduct consumed amount from inventory
        max_mat = inv.level()
        mat_used, _ = d.fulfill(max_mat, allow_part_shipments) 
        inv.get(mat_used)
